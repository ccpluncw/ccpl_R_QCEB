...
#' @param thresholdWindowSize An integer specifying the number of frames (i.e., trials) on which you want to base your speedFeedback threshold. The program will take the most recent "thresholdWindowSize" frames to create the RTwindowArray. The RTwindowArray will be used, in part, to determine the RT threshold of the next trial. DEFAULT = 20.
#' @param thresholdArrayPosition An integer specifying the array position in the RTwindowArray, sorted from fastest to slowest, to use as the new potential RT threshold. So, if the thresholdArrayPosition = 18 and thresholdWindowSize = 20, then the 18th slowest RT would be the new potential RT threshold. DEFAULT = 17.
...
#' @param initialWindowMessageDisplay A string that specifies the message to be shown to the participant after every trial during the initial period while the first "thresholdWindowSize" trials are being accumulated. The string must be in html format.  You can use any html codes.  DEFAULT = "&nbsp" (blank).
#' @param initialWindowMessageOutput A string that will be output in the datafile every trial during the initial period while the first "thresholdWindowSize" trials are being accumulated. DEFAULT = "initial".
#' @param initialStimulusDuration An integer specifying the time, in ms, that the feedback message should be presented during the initial period while the first "thresholdWindowSize" trials are being accumulated. DEFAULT = 1 (not noticable)
#' @param initialPostTrialGap An integer specifying the time, in ms, between the feedback message and the next frame during the initial period while the first "thresholdWindowSize" trials are being accumulated. DEFAULT = 1 (not noticable)
...
buildSpeedFeedbackList <- function (showFeedback = TRUE, speedThresholdUp = TRUE, frameName = "test", thresholdWindowSize = 20, thresholdArrayPosition = 17, thresholdBoundary = NULL, currentRTWindowSize = 1, initialWindowRtThreshold = NULL, initialWindowMessageDisplay = "&nbsp", initialWindowMessageOutput = "initial", initialStimulusDuration = 1, initialPostTrialGap = 1, aboveThresholdMessageDisplay = NULL, aboveThresholdMessageOutput = "above", aboveStimulusDuration = NULL, abovePostTrialGap= NULL, belowThresholdMessageDisplay = NULL, belowThresholdMessageOutput = "below", belowStimulusDuration = NULL, belowPostTrialGap = NULL) {
  ...
  if(!isSingleNumeric(thresholdWindowSize)) {
    stop("thresholdWindowSize option must be a single integer.  Yours, apparently, is not.")
  }

  if(!isSingleNumeric(currentRTWindowSize)) {
    stop("currentRTWindowSize option must be a single integer.  Yours, apparently, is not.")
  }

  if((currentRTWindowSize > thresholdWindowSize)) {
    stop("currentRTWindowSize option must be less than or equal to thresholdWindowSize.  Yours, apparently, is not.")
  }
