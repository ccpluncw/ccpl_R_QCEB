...
#' @param getUserNameFile  A string that specifies the name of the html file that collects the user's identifying information (e.g., a random number). DEFAULT = NULL. INERT: the value is accepted and written to the config for backward compatibility, but current engines do not act on it -- no such screen is presented.
...
#' @param restMsg A string that specifies the rest message to be shown at the beginning of a break. The string must be in html format.  You can use any html codes.  DEFAULT = NULL. INERT at this level: the value is accepted and written to the config for backward compatibility, but current engines do not act on it -- rests, and their message, are configured at the block level.
...
buildQCEexpDbFile <- function (expName = "defaultExpName", addQualtricsCode = FALSE, defaultBackgroundColor = "#000000", restAfterEveryNTrials = -1, instructionFile = NULL, getUserNameFile = NULL, getConsentFile = NULL, getDemographicsFile = NULL, getGenderFile = NULL, welcomeMsg = NULL, restMsg = NULL, endOfSessionMsg = NULL, endOfExpMsg = NULL, saveMsg = NULL, closeBrowserMsg = NULL, fullscreenMsg = NULL, fullscreenBtn = "Continue", completionRedirect = NULL, saveDataEveryNTrials = 50, completionGate = NULL, maxExperimentMinutes = NULL, saveTimeoutMs = NULL, saveCanary = NULL, saveUnavailableMsg = NULL, warnOnLeave = NULL, strictGroupAssignment = NULL, creditClaimTimeoutMs = NULL) {
  ...
  if (!is.null(completionGate)) {
    ...
    # Who scores the run. The engine accepts exactly two values and refuses
    # anything else at session start, so a wrong value here is refused at
    # build time instead of in front of a participant.
    if (!is.null(completionGate$scoredBy) &&
        (!isSingleString(completionGate$scoredBy) ||
         !(completionGate$scoredBy %in% c("server", "client")))) {
      stop("completionGate$scoredBy must be \"server\" or \"client\", or absent (client-scored).")
    }

    # Participant-facing messages. None is required -- the engine supplies a
    # default for each except creditMsg, whose absence means no grant screen at
    # all -- but an EMPTY one is a different thing from an absent one: it
    # reaches the participant as a blank screen at the moment they are being
    # told what happened to their credit.
