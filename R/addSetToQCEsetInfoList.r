...
#' @param excludePreviouslyPresented Optional single boolean. When TRUE, this set's pool is filtered at runtime to exclude any scenarioID the participant has already been shown anywhere in the experiment. Honored on regular blocks as well as on sets built inside switchRules blocks: the flag is OR-ed with the block-level exclusion flag, so setting it here filters this set's pool even when the block itself does not ask for filtering. Phase 3 Decision 6 (Semantic C) -- the flag is a property of the destination set's pool, not of the firing rule, so any path that builds the set (rule firing, natural fallthrough, recursive next-natural) honors it consistently. NULL means do not filter. DEFAULT = NULL.
...
#' sl <- addSetToQCEsetInfoList(sl, scenarios, "followUp", 15,
#'   showIf = buildQCEshowIfCondition("eligible", "equals", "Yes", "Response"))
#'
